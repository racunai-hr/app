...
import { tenantApiOrigin } from '@/lib/assets';

export type AssetsSession = {
  tenant: TenantInfo;
  origin: string;
  token: string;
  role: string;
...
export function useAssetsSession(slug: string): {
  session: AssetsSession | null;
  loading: boolean;
  error: string;
} {
  const router = useRouter();
  const [session, setSession] = useState<AssetsSession | null>(null);
